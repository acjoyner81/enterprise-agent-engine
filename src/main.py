...
def run_vectorless_rag_demo():
    ...
    if result["found"]:
        console.print(
            f"[bold green]✓ Found Enterprise Record: {record_id}[/bold green]"
        )
...
def run_guardrail_demo():
    ...
    table.add_row(
        "Input Allowed",
        "[green]YES[/green]" if decision.allowed else "[bold red]BLOCKED[/bold red]",
    )
    ...
    table.add_row(
        ...
        ", ".join(decision.injection_result.detected_patterns) or "None",
    )
    table.add_row("PII Detected", str(decision.pii_result.has_pii))
    table.add_row(
        "Redacted PII Types", ", ".join(decision.pii_result.redacted_types) or "None"
    )
...
def run_benchmark_evals():
    ...
    for cat, stats in summary.category_scores.items():
        table.add_row(
            cat, str(stats["total"]), str(stats["passed"]), f"{stats['accuracy_pct']}%"
        )
    ...
    table.add_row(
        ...
        f"[bold]{summary.accuracy_pct}%[/bold]",
    )

    console.print(table)
    console.print(
        f"[bold green]✓ Benchmark Accuracy: {summary.accuracy_pct}% | Avg Latency: {summary.avg_latency_ms} ms[/bold green]\n"
    )
...
def run_langgraph_demo(user_prompt: str = "Analyze system health and JVM load."):
    ...
    table.add_row(
        "Security Evaluation",
        "[green]PASSED[/green]" if payload.is_safe else "[red]BLOCKED[/red]",
    )
...
def check_multica_status():
    ...
    if multica_path:
        try:
            ...
            console.print(
                f"[bold green]✓ Multica CLI detected:[/bold green] {multica_path}"
            )
            console.print(f"  [cyan]{ver}[/cyan]")
        except Exception as exc:  # noqa: BLE001
            console.print(
                f"[yellow]Multica detected at {multica_path}, error reading version: {exc}[/yellow]"
            )
    else:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if arg == "--serve-mcp":
            console.print(
                "[bold green]Starting FastMCP Server on stdio transport...[/bold green]"
            )
            mcp.run()
            return
        elif arg == "--eval":
            ...
            prompt = (
                sys.argv[2]
                if len(sys.argv) > 2
                else "Analyze system health and JVM load."
            )
    ...
    check_multica_status()
    console.print(
        "[bold green]✓ Epics 1, 2, & 3 operational and verified in Multica![/bold green]\n"
    )
