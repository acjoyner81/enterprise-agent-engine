...
async def test_concurrent_agent_gateway_stress():
    ...
    gateway = LiteLLMGateway(
        primary_model="nonexistent/load-test", enable_mock_fallback=True
    )
    ...
    async def single_agent_task(idx: int):
        ...
        resp = await gateway.agenerate(
            messages=[
                {
                    "role": "user",
                    "content": f"Execute load query #{idx} for fulfillment cluster.",
                }
            ],
    ...
    results = await asyncio.gather(
        *[single_agent_task(i) for i in range(concurrency_count)]
    )
    _total_batch_ms = (time.perf_counter() - start_batch) * 1000
